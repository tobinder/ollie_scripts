...
#Only 200 wf2 jobs are allowed in queue, delay submission of further jobs
date=20180508
segment=01
echo ----------------------------------
...
do  
  while true; do
          if test $(squeue | grep tbinder | grep "smp b" |  wc -l) -gt 199; then
            sleep 60
            else break
          fi
      done
  if [ $wf -eq 2 ]; then
    if test -e '/work/ollie/tbinder/Scratch/rds/2018_Greenland_Polar6/CSARP_out/'$date'_'$segment'/fk_data_'$frm'_01_01/wf_02_adc_'$adc'_chk_'$chunk'.mat'
            then
             ...
             sbatch -J 'b'${frm#0}''$chunk''$adc ./create_csarp_task_wf2_smp.sh $static_param $dynamic_param $frm $chunk $wf $adc
    fi
  fi
  sleep 1 
done < $file_tmp
