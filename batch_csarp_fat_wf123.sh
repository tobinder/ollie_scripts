  ...
  if [ $wf -eq 1 ]; then
    if test -e '/work/ollie/tbinder/Scratch/rds/2018_Greenland_Polar6/CSARP_out/'$date'_'$segment'/fk_data_'$frm'_01_01/wf_01_adc_'$adc'_chk_'$chunk'.mat'
            then
             ...
             sbatch -J 'a'${frm#0}''$chunk''$adc ./create_csarp_task_wf1.sh $static_param $dynamic_param $frm $chunk $wf $adc
    fi
  fi 
  if [ $wf -eq 2 ]; then
    if test -e '/work/ollie/tbinder/Scratch/rds/2018_Greenland_Polar6/CSARP_out/'$date'_'$segment'/fk_data_'$frm'_01_01/wf_02_adc_'$adc'_chk_'$chunk'.mat'
            then
             echo 'File fk_data_'$frm'_01_01/wf_02_adc_'$adc'_chk_'$chunk'.mat already exists.'
            else 
             sbatch -J 'b'${frm#0}''$chunk''$adc ./create_csarp_task_wf2.sh $static_param $dynamic_param $frm $chunk $wf $adc
    fi
  fi
  if [ $wf -eq 3 ]; then
